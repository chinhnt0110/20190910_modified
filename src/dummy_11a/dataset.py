...
class SteelDataset(Dataset):
    def __init__(self, split, csv, mode, augment=None):
        ...
        df['ImageId_ClassId'] = df['ImageId'].map(str) + '_' + df['ClassId'].map(str)
        print(len(df['ImageId_ClassId']))
        image_id_class_id = [] # from train_images
        encoded_pixel = []
        for image_id in [u.split('/')[-1] for u in self.uid]:
            # for c in range(4):
            for c in range(1):
                if image_id + '_%d' % (c + 1) not in image_id_class_id:
                    image_id_class_id.append(image_id + '_%d' % (c + 1))

        for iici in image_id_class_id:
            ind_list = df[df['ImageId_ClassId'] == iici].index.values
            if len(ind_list) != 0:
                encoded_pixel.append(df['EncodedPixels'][ind_list[0]])
            else:
        ...
        df = df_loc_by_list(df, 'ImageId_ClassId',
                            [u.split('/')[-1] + '_%d' % c for u in self.uid for c in [1]])
        self.df = df
    ...
    def __getitem__(self, index):
        ...
        image = cv2.resize(image, (1216, 800))
        mask  = np.array([run_length_decode(r, height=800, width=1216, fill_value=1) for r in rle])
