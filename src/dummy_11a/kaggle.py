...
# ROOT_DIR = /mnt/ssd1/projects/steel-kaggle/20190910

# https://www.kaggle.com/iafoss/severstal-fast-ai-256x256-crops-sub
# https://www.kaggle.com/rishabhiitbhu/unet-starter-kernel-pytorch-lb-0-88F

# DATA_DIR = ROOT_DIR + '/data/scratch/'
...
def run_check_rle():
    ...
    image = cv2.imread(ROOT_DIR + '/data/train_images/002fc4e19.jpg',cv2.IMREAD_COLOR)
    value = [
...
def run_make_split():
    ...
    np.save(ROOT_DIR + '/data/split/train0_%d.npy'%len(train),train)
    np.save(ROOT_DIR + '/data/split/valid0_%d.npy'%len(valid),valid)
...
def run_make_train_split():
    ...
    for n in range(num_fold):
        ...
        np.save(ROOT_DIR + '/data/split/train_a%d_%d.npy'%(n,len(train)),train)
        np.save(ROOT_DIR + '/data/split/valid_a%d_%d.npy'%(n,len(valid)),valid)


def run_make_test_split():

    df =  pd.read_csv(ROOT_DIR + '/data/sample_submission.csv')
    df['ImageId'] = df['ImageId_ClassId'].apply(lambda x: x.split('_')[0])
    uid = df['ImageId'].unique().tolist()

    test = ['test_images/'+i for i in uid]
    np.save(ROOT_DIR + '/data/split/test_%d.npy'%len(test),test)
...
def run_make_test_split1():
    ...
    np.save(ROOT_DIR + '/data/split/test_train_%d.npy'%len(valid),valid)
    np.save(ROOT_DIR + '/data/split/test_valid_%d.npy'%len(train),train)
...
def run_make_dummy():
    ...
    for i in image_id:
        ...
        image = cv2.imread(ROOT_DIR + '/data/train_images/%s'%(i), cv2.IMREAD_COLOR)
        mask  = np.array([run_length_decode(r, height=256, width=1600, fill_value=1) for r in rle])
