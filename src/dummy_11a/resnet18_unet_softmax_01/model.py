...
class Net(nn.Module):
    ...
    def __init__(self, num_class=2, drop_connect_rate=0.2):
...
def one_hot_encode_truth(truth, num_class=1):
...
def one_hot_encode_predict(predict, num_class=1):
...
def criterion(logit, truth, weight=None):
    # print('******', logit.shape)
    logit = logit.permute(0, 2, 3, 1).contiguous().view(-1, 2)
