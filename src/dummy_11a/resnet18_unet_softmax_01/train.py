...
def null_collate(batch):
    ...
    for b in range(batch_size):
        ...
        mask = batch[b][1]
        # if len(np.unique(mask, return_counts=True)[1]) < 2:
        #     continue
        label = (mask.reshape(1, -1).sum(1) > 0).astype(np.int32)
...
def do_valid(net, valid_loader, out_dir=None):
    ...
    for t, (input, truth_mask, truth_label, infor) in enumerate(valid_loader):
        ...
        print('truth_mask_shape', truth_mask.shape)
...
def run_train():
    out_dir = ROOT_DIR + '/result/scratch/resnet18-seg'

    # initial_checkpoint = ROOT_DIR + '/result/scratch/resnet18-seg/checkpoint/00121500_model.pth'
    ...
    log.write('rate     iter   epoch |  loss    hit_neg,pos1  dice_neg,pos1|  loss    hit_neg,pos1 | time         \n')
    log.write('---------------------------------------------------------------------------------------------------\n')
    # 0.00000    0.0*   0.0 |  0.690   0.50 [0.00]   0.44 [0.00]  |  0.000   0.00 [0.00]  |  0 hr 00 min
    ...
    while iter < num_iters:
        ...
        for t, (input, truth_mask, truth_label, infor) in enumerate(train_loader):
            ...
            if (iter % iter_log == 0):
                ...
                log.write(
                    '%0.5f  %5.1f%s %5.1f |  %5.3f   %4.2f [%4.2f]   %4.2f [%4.2f]  |  %5.3f   %4.2f [%4.2f]  | %s' % ( \
            ...
            print('\r', end='', flush=True)
            asterisk = ' '
            print('%0.5f  %5.1f%s %5.1f |  %5.3f   %4.2f [%4.2f]   %4.2f [%4.2f]  |  %5.3f   %4.2f [%4.2f]  | %s' % ( \
                rate, iter / 1000, asterisk, epoch,
                *valid_loss[:5],
                *batch_loss[:3],
                time_to_str((timer() - start), 'min'))
