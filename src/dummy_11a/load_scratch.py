...
def check_valid_mask(image):
    image = cv2.resize(image, (800, 1216))
    indices = np.argwhere(image == [255])
    if len(indices) == 0:
        return True
